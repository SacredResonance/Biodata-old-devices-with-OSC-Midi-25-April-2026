...
// MIDI channel / IP
byte channel = 6;   // STATIC MIDI CHANNEL: change this to 1, 2, 3, 4, or 5 as needed
IPAddress local_IP(192,168,8,55); //50// 51 // *** DONT FORGET TO CHANGE *** 
bool staticIP = true;
...
unsigned int oscOutPort = 8005;           // Max: use [udpreceive 8000]
WiFiUDP oscUdp;
...
void setupWifi() {
  ...
  Serial.print("WiFi IP address set in code before connect: ");
  Serial.println(local_IP);

  Serial.print("OSC target IP before connect: ");
  Serial.println(oscTargetIP);
  Serial.print("OSC output port before connect: ");
  Serial.println(oscOutPort);
  ...
  if (oscMIDI) {
    oscUdp.begin(oscOutPort);
    Serial.print("OSC enabled to ");
...
// OSC functions - simple OSC 1.0 message writer, no extra library needed
void oscWritePaddedString(const char *str)
{
  int len = strlen(str) + 1;
  oscUdp.write((const uint8_t*)str, strlen(str));
  oscUdp.write((uint8_t)0);
  while (len % 4 != 0) {
    oscUdp.write((uint8_t)0);
    len++;
  }
...
void oscWriteInt32(int32_t value)
{
  uint8_t b[4];
  b[0] = (value >> 24) & 0xFF;
  b[1] = (value >> 16) & 0xFF;
...
void sendOSC3Int(const char *address, int a, int b, int c)
{
  if (!oscMIDI) return;
  if (WiFi.status() != WL_CONNECTED) return;

  oscUdp.beginPacket(oscTargetIP, oscOutPort);
  oscWritePaddedString(address);
  oscWritePaddedString(",iii");
  oscWriteInt32(a);
  oscWriteInt32(b);
  oscWriteInt32(c);
  oscUdp.endPacket();
...
void sendOSCNoteOn(int note, int velocity, int notechannel)
{
  // Max receives: /biodata/noteon channel note velocity
  sendOSC3Int("/biodata/noteon", notechannel, note, velocity);
}

void sendOSCNoteOff(int note, int velocity, int notechannel)
{
  // Max receives: /biodata/noteoff channel note velocity
  sendOSC3Int("/biodata/noteoff", notechannel, note, velocity);
}

void sendOSCControlChange(int ccNumber, int ccValue, int ccChannel)
{
  // Max receives: /biodata/cc channel ccNumber ccValue
  sendOSC3Int("/biodata/cc", ccChannel, ccNumber, ccValue);
}
...
void setNote(int value, int velocity, long duration, int notechannel)
{
  for (int i = 0; i < polyphony; i++) {
    if (!noteArray[i].velocity) {
      ...
      sendOSCNoteOn(value, velocity, notechannel);
...
void checkControl()
{
  ...
  if (distance != 0) {
    if (currentMillis > controlMessage.duration) {
      ...
      sendOSCControlChange(controlMessage.type, controlMessage.value, channel);
    }
  }
}
  ...
  for (int i = 0; i < polyphony; i++) {
    if (noteArray[i].velocity) {
      if (noteArray[i].duration <= currentMillis) {
        ...
        sendOSCNoteOff(noteArray[i].value, 0, noteArray[i].channel);
